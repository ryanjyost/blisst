<?php

// =============================================================================
// CATEGORY_TEMPLATE.PHP
// -----------------------------------------------------------------------------
// This code feeds through to all of the individual PARENT category archive pages
// =============================================================================

?>


<!--  Main sidebar -->
<div class="main-side">
     <?php x_get_view( 'global', 'main-side' ); ?>
</div>


<!--  Middle Category Section w/ loop -->
<div id="cat-middle" class= "column-middle">
  <?php x_get_view( 'global', 'category-middle' ); ?>
</div>


<div id="cat-right" class="column-right">
    <?php x_get_view( 'global', 'category-right' ); ?>
</div>
















